...
## 🧭 개요
에이전트가 외부 세계와 만나는 통로인 **도구**. 이론에서 실행의 분업(모델은 호출 의도를 텍스트로, 실행은 코드가), 프롬프트 규약과 **function calling**, 스키마 설계, 실패의 재주입을 다룬다. 실습은 도구 레지스트리·파싱·실행의 단발 왕복. 다음 주 이 왕복이 루프가 된다. Toolformer·ToolLLM.
...
*이번 주 주제:* 도구 레지스트리·파싱·실행 직접 구현 (계산기 등) — 단발 왕복
