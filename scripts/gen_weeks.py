...
# 순서: 개념 의존성 기준 v6 (W3=추론 모델과 강화학습, SC/TTC는 W2, RAG 통합 W6, MCP W11 — docs/syllabus.md 참고).
# week: (part, title, aux, practice, discussion, [papers])
# paper: (diff, title, cite, must, deep, pdf, optional)
W = {
1:("부품에서 루프까지","강의소개 · 에이전트 개요 (What is an Agent?)",
 ...
 []),
2:("부품에서 루프까지","프롬프팅과 추론 (Prompting & Reasoning)",
 "Anthropic — *Prompt Engineering Interactive Tutorial* · Berkeley `llm-reasoning.pdf`",
 "직접 질의 vs CoT 정답률 측정 + few-shot exemplar 작성으로 정답률 개선",
 ...
 [("🟢","Chain-of-Thought Prompting Elicits Reasoning in LLMs","Wei et al., NeurIPS 2022 · arXiv:2201.11903",
   "CoT 핵심 아이디어와 추론 창발 조건","규모별 창발 곡선","W02_Chain-of-Thought_2201.11903.pdf",False)]),
3:("부품에서 루프까지","추론 모델과 강화학습 (Reasoning Models & RL)",
 "Lilian Weng — *Why We Think* · Berkeley `Jason-Weston-Reasoning.pdf`",
 "self-consistency(N샘플 다수결) 구현 — 추론 모델이 내재화한 호출자 쪽 반복 절차의 실물, N=1 대비 정확도·비용 비교",
...
4:("부품에서 루프까지","도구 사용 (Tool Use)",
 "Anthropic — *Writing Effective Tools for Agents* · MS04",
 "도구 레지스트리·파싱·실행 직접 구현 (계산기 등) — 단발 왕복",
...
5:("부품에서 루프까지","에이전트 루프 (ReAct)",
 "Lilian Weng — *LLM Powered Autonomous Agents* · Berkeley 에이전트 역사 강의",
 "ReAct(Thought→Action→Observation) while 루프 직접 구현 — W4 도구 레지스트리 위에서 + 미니 evalset",
 ...
 [("🟢","ReAct: Synergizing Reasoning and Acting in LLMs","Yao et al., ICLR 2023 · arXiv:2210.03629",
   "Think–Act–Observe 루프 구조와 추론·행동을 엮는 이유","HotpotQA·ALFWorld 셋업","W05_ReAct_2210.03629.pdf",False)]),
6:("지식","검색 증강 (RAG) — 기초와 에이전틱",
 "Anthropic — *Contextual Retrieval* · MS05 · `RAG_Techniques`(기초 노트북)",
 "임베딩→코사인 top-k 검색 구현 + 검색을 도구로 등록 *(최종 프로젝트 부품 1·2)*",
 ...
 [("🟡","Retrieval-Augmented Generation for Knowledge-Intensive NLP","Lewis et al., NeurIPS 2020 · arXiv:2005.11401",
  ...
  ("🟡🔴","Self-RAG: Learning to Retrieve, Generate, and Critique","Asai et al., ICLR 2024 · arXiv:2310.11511",
   "reflection token으로 검색 여부·품질을 스스로 판단","critic 학습, segment beam search","W06_Self-RAG_2310.11511.pdf",False),
  ("🟡","HyDE: Precise Zero-Shot Dense Retrieval (선택읽기)","Gao et al., ACL 2023 · arXiv:2212.10496",
   "가설 문서를 생성해 검색 품질을 올리는 발상","dense retriever와의 결합","W06_opt-HyDE_2212.10496.pdf",True),
  ("🟡","Adaptive-RAG (선택읽기)","Jeong et al., NAACL 2024 · arXiv:2403.14403",
   "질의 난이도에 따라 검색 전략을 바꾸는 에이전틱 구조","complexity classifier","W06_opt-Adaptive-RAG_2403.14403.pdf",True)]),
7:("단일 에이전트 고도화","계획과 탐색 (Planning & Search)",
 "LangChain — *Plan-and-Execute Agents* · MS07",
 "멀티홉 검색 질문 분해 플래너, 계획–실행 분리",
 "탐색 비용 대비 성능 이득은 언제 정당한가?",
 [("🟡","Tree of Thoughts: Deliberate Problem Solving with LLMs","Yao et al., NeurIPS 2023 · arXiv:2305.10601",
   "사고를 트리로 분기·탐색(BFS/DFS)하는 발상","상태 평가 함수, Game of 24","W07_Tree-of-Thoughts_2305.10601.pdf",False),
  ("🟡","ReWOO: Decoupling Reasoning from Observations","Xu et al., 2023 · arXiv:2305.18323",
   "계획을 관찰과 분리해 토큰·호출을 줄이는 구조","planner/worker/solver 모듈 분해","W07_ReWOO_2305.18323.pdf",False)]),
8:("단일 에이전트 고도화","자기반성·메타인지 (Reflection)",
 "Anthropic — *Demystifying Evals for AI Agents* · MS09",
 "검색 실패→언어 피드백→재시도 루프를 ReAct에 추가",
 "자기반성은 진짜 개선인가, 아니면 같은 오류의 반복인가?",
 [("🟡","Reflexion: Language Agents with Verbal RL","Shinn et al., NeurIPS 2023 · arXiv:2303.11366",
   "언어 피드백이 gradient 없이 학습되는 메커니즘","actor-evaluator-reflection 구조","W08_Reflexion_2303.11366.pdf",False),
  ("🟢","Self-Refine: Iterative Refinement with Self-Feedback","Madaan et al., NeurIPS 2023 · arXiv:2303.17651",
   "단일 모델의 생성→비평→개선 반복","과제별 개선 폭, 피드백 프롬프트 설계","W08_Self-Refine_2303.17651.pdf",False)]),
9:("단일 에이전트 고도화","컨텍스트 엔지니어링 (하네스) ⭐",
...
11:("협업·환경","멀티에이전트 + LangGraph + MCP 전환점 ⭐",
...
2:"에이전트의 추론 능력을 프롬프트와 계산으로 끌어내는 주. 이론에서 추론의 정의, 직접 질의가 실패하는 원인(글밖에 없는 작업 공간), **CoT**(중간 단계의 외부화 — 지시와 풀이 예시), 그 확장인 **self-consistency**(표집과 다수결)와 **test-time compute**(예측 횟수·정확도의 교환)를 다룬다. 실습은 직접 질의 vs CoT 정답률 측정과 few-shot exemplar 작성(다수결 구현은 W3 실습). **첫 학생 발표 주 — 부담 완화를 위해 1편만.**",
3:"추론 능력을 모델의 가중치에 넣는 주. 이론에서 W2 test-time compute의 내재화인 **추론 모델**(o1/R1, thinking budget), 훈련 데이터의 문제(과정 레이블 부재), **STaR**(자가 생성·정오 필터), **강화학습**(정책·보상, RLHF vs RLVR), **R1**(GRPO, 창발)과 보상 해킹·증류를 다룬다. **RL 사전 지식 없음 가정** — 정의부터 강의에서 세운다. 실습은 self-consistency 구현(호출자 쪽 반복 절차의 실물)과 N=1 대비 비교.",
4:"에이전트가 외부 세계와 만나는 통로인 **도구**. 이론에서 실행의 분업(모델은 호출 의도를 텍스트로, 실행은 코드가), 프롬프트 규약과 **function calling**, 스키마 설계, 실패의 재주입을 다룬다. 실습은 도구 레지스트리·파싱·실행의 단발 왕복. 다음 주 이 왕복이 루프가 된다. Toolformer·ToolLLM.",
5:"도구 왕복을 반복 구조로 확장해 **첫 완전한 에이전트**를 완성하는 주. 이론에서 결정권의 이양(에이전트 루프), 행동만 반복하는 구성(Act-only)의 실측 실패와 진단, 처방으로서의 **ReAct**를 다루고, 트레이스 리딩과 루프 가드를 익힌다. 이후 매주 쓰는 **미니 evalset**(5문항)을 시작한다. ReAct.",
6:"외부 지식을 끌어오는 **검색 증강(RAG)** 을 기초부터 에이전틱까지 한 주에 다룬다. W5의 에이전트는 돌지만 아는 것이 없다 — 이론에서 임베딩·코사인 유사도(이 과목 유일의 수식), 청킹→인덱스→top-k 파이프라인, 그리고 고정 파이프라인의 낭비에서 **검색=도구**(에이전틱 RAG)로의 전환을 다룬다. 실습에서 retriever를 만들고 곧바로 도구로 등록한다 *(최종 프로젝트 부품 1·2)*. RAG(Lewis)·Self-RAG. *(보안 복선: 오늘 만든 '문서 입구'가 W14 간접 인젝션의 공격 통로가 된다 — 예고만.)*",
7:"복잡한 과제를 나누고 탐색하는 **계획**. ReAct의 greedy 진행은 멀티홉 검색 질문에서 무너진다 — 이론에서 과제 분해, 계획–실행 분리, **트리 탐색 직관**을 다루고, 실습에서 멀티홉 검색 질문을 분해하는 플래너를 W6 검색 도구 위에 구현한다. Tree of Thoughts·ReWOO.",
8:"자기 출력을 스스로 고치는 **반성·메타인지**. 계획(W7)을 세워도 첫 시도는 틀린다 — 이론에서 생성→비평→개선 루프와 **언어 피드백이 gradient 없이 작동하는 원리**를 다루고, 실습에서 검색 실패→피드백→재시도를 루프에 붙인다. Reflexion·Self-Refine.",
...
PRACTICE = {
1:"**추가 모듈:** `llm.py` — (원시 API 1회 호출로 내부 확인 후) **aisuite**로 provider-무관 `chat()` 래퍼.\n진행: 실습 노트북(`lectures/week01/W1_lab_setup.ipynb`) → `pytest tests/test_week01.py` → `demos/week01_raw_api.py`\n> ✅ **완료:** 원시 HTTP와 내가 채운 `chat()` 양쪽으로 같은 질문에 답을 받는다 + 형식 준수율 개선 기록.",
2:"**실험:** `reasoning.py`의 `COT_PROMPT`로 직접 질의 vs CoT 정답률을 측정하고, few-shot exemplar를 직접 작성해 정답률을 올린다.\n진행: 실습 노트북(`lectures/week02/W2_lab_prompting.ipynb`) → `demos/week02_cot.py`\n> ✅ **완료:** 직접 질의 대비 CoT, zero-shot 대비 exemplar의 정답률 표를 기록한다.",
3:"**추가 모듈:** `reasoning.py` — 같은 질문을 N번 샘플→다수결(`majority_vote`·`self_consistency`).\n진행: 실습 노트북(`lectures/week03/W3_lab_selfconsistency.ipynb`) → `pytest tests/test_week03.py` → `demos/week03_selfconsistency.py`\n> ✅ **완료:** 애매한 질문에서 N=1보다 N=5의 정확도가 높고, 그 대가로 비용이 N배임을 함께 기록한다.",
4:"**추가 모듈:** `tools.py` — 도구 등록(dict)·JSON 액션 파싱·실행(계산기·문자열검색).\n> ✅ **완료:** 모델의 도구 호출 출력을 파싱·실행해 결과를 재주입하는 단발 왕복이 성공한다. (반복 루프는 W5)",
5:"**추가 모듈:** `loop.py` — ReAct while 루프 골격. W4의 `tools.py` 레지스트리를 호출한다. + 미니 evalset 5문항.\n> ✅ **완료:** 루프가 계산기 도구를 실제로 호출해 답하고, 미니 evalset 결과가 기록된다.",
6:"**추가 모듈:** `retriever.py` — 청킹→임베딩→코사인 top-k 검색, 그리고 `tools.py`에 검색 도구로 등록. 📦 *최종 부품*\n> ✅ **완료:** 문서 속 사실을 물으면 관련 청크로 답하고, 상식 질문은 검색하지 않는다.",
7:"**추가 모듈:** `planner.py` — 질문을 하위 단계 리스트로 분해→순차 실행 (W6 검색 도구 사용).\n> ✅ **완료:** 2단계 검색 질문을 계획대로 처리한다.",
8:"**추가 모듈:** `reflect.py` — 검증 실패 시 언어 피드백을 붙여 재시도(최대 k회). 피드백 소스는 검색 품질·도구 에러.\n> ✅ **완료:** 처음 틀린 답을 재시도로 고친다.",
...
W1_PAPERS = ("**⚠️ 학생 발표 없음** — 발표 배정 전이다. 이 시간에 **W2~W15 발표자·지정 토론자를 일괄 배정**한다.\n"
 ...
 "6장 템플릿·15분 하드 스톱 그대로 시연. 수업 발표 목록에 없는 논문이라 스포일러가 없고, W2(프롬프팅과 추론) 이론·실습의 예고편이 된다.\n"
 "- 다음 주 발표 논문(CoT — 첫 발표 주라 1편만)은 **개강 전 주에 발표자에게 미리 공지**한다.\n")
