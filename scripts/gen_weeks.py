...
W = {
1:("에이전트 기초","에이전트란 무엇인가 — 추론과 행동의 결합",
 ...
 [("🟢","ReAct: Synergizing Reasoning and Acting in LLMs","Yao et al., ICLR 2023 · arXiv:2210.03629",
   "Think–Act–Observe 루프 구조와 추론·행동을 엮는 이유","HotpotQA·ALFWorld 셋업과 프롬프트 구성","W01_ReAct_2210.03629.pdf",False),
...
2:("에이전트 기초","도구 사용 (Tool Use)",
 ...
 [("🟡","Toolformer: LMs Can Teach Themselves to Use Tools","Schick et al., NeurIPS 2023 · arXiv:2302.04761",
   "self-supervised로 API 호출 위치를 학습하는 방식","호출 필터링 손실, 데이터 생성 파이프라인","W02_Toolformer_2302.04761.pdf",False),
  ("🟡","ToolLLM: Mastering 16000+ Real-world APIs","Qin et al., ICLR 2024 · arXiv:2307.16789",
   "대규모 실세계 API 학습 프레임과 DFSDT 탐색","ToolBench 구축, pass/win rate","W02_ToolLLM_2307.16789.pdf",False)]),
3:("에이전트 기초","피드백·자기반성 (Reflection)",
 ...
 [("🟡","Reflexion: Language Agents with Verbal RL","Shinn et al., NeurIPS 2023 · arXiv:2303.11366",
   "언어 피드백이 gradient 없이 학습되는 메커니즘","actor-evaluator-reflection 구조, 메모리 버퍼","W03_Reflexion_2303.11366.pdf",False),
  ("🟢","Self-Refine: Iterative Refinement with Self-Feedback","Madaan et al., NeurIPS 2023 · arXiv:2303.17651",
   "단일 모델의 생성→비평→개선 반복","과제별 개선 폭, 피드백 프롬프트 설계","W03_Self-Refine_2303.17651.pdf",False)]),
4:("에이전트 기초","계획과 탐색 (Planning & Search)",
...
5:("추론과 지식","🆕 추론 모델 (Reasoning Models / Test-time Compute)",
 ...
 [("🟡","STaR: Self-Taught Reasoner","Zelikman et al., NeurIPS 2022 · arXiv:2203.14465",
   "스스로 만든 추론으로 추론을 부트스트랩하는 아이디어","rationalization 트릭","W05_STaR_2203.14465.pdf",False),
  ("🟡","DeepSeek-R1: Incentivizing Reasoning via RL","DeepSeek-AI, 2025 · arXiv:2501.12948",
   "순수 RL로 추론이 창발하는 큰 그림('aha moment')","GRPO, cold-start 데이터","W05_DeepSeek-R1_2501.12948.pdf",False)]),
6:("추론과 지식","검색 증강 (RAG) — 1부: 기초",
...
7:("추론과 지식","검색 증강 (RAG) — 2부: 에이전틱 RAG",
...
8:("추론과 지식","메모리 (Memory)",
 ...
 [("🟡","MemGPT: Towards LLMs as Operating Systems","Packer et al., 2023 · arXiv:2310.08560",
   "가상메모리 비유의 계층적 메모리(core/archival)","function-call self-editing, 페이징","W08_MemGPT_2310.08560.pdf",False),
  ("🟡","MemoryBank: Enhancing LLMs with Long-Term Memory","Zhong et al., AAAI 2024 · arXiv:2305.10250",
   "망각 곡선 기반 메모리 갱신·검색","사용자 페르소나 유지","W08_MemoryBank_2305.10250.pdf",False)]),
9:("협업과 환경","체화·평생학습 (Embodied / Lifelong)",
 ...
 [("🟡","Voyager: An Open-Ended Embodied Agent with LLMs","Wang et al., 2023 · arXiv:2305.16291",
   "스킬 라이브러리를 누적하는 평생학습 메커니즘","automatic curriculum, 코드형 스킬","W09_Voyager_2305.16291.pdf",False),
  ("🟡","Generative Agents: Interactive Simulacra of Human Behavior","Park et al., UIST 2023 · arXiv:2304.03442",
   "메모리·반성·계획을 한 시스템에 통합","최신성·중요도·관련성 점수화","W09_Generative-Agents_2304.03442.pdf",False)]),
10:("협업과 환경","멀티에이전트 + LangGraph 전환점 ⭐",
 ...
 [("🟡","AutoGen: Multi-Agent Conversation Framework","Wu et al., COLM 2024 · arXiv:2308.08155",
  ...
  ("🟡","Multiagent Debate (선택읽기)","Du et al., ICML 2024 · arXiv:2305.14325",
   "다중 에이전트 토론이 사실성·추론을 높이는 원리","수렴 동역학, 에이전트 수·라운드","W10_opt-Multiagent-Debate_2305.14325.pdf",True),
  ("🟡","CAMEL: Communicative Agents (선택읽기)","Li et al., NeurIPS 2023 · arXiv:2303.17760",
   "역할극 기반 자율 협력 프레임","inception prompting, 과제 분해","W10_opt-CAMEL_2303.17760.pdf",True)]),
11:("프런티어 (개념 위주)","에이전트 강화학습 (개념 1주 · 수식 생략)",
 ...
 [("🔴","ReTool: RL for Strategic Tool Use in LLMs","2025 · arXiv:2504.11536 · *수식 유도 생략 가능*",
   "도구 사용 시점·방법을 RL로 최적화하는 핵심 직관","코드 인터프리터 통합, outcome 보상","W11_ReTool_2504.11536.pdf",False),
  ("🔴","Agentic Context Engineering (선택읽기·자기진화)","2025 · arXiv:2510.04618",
   "컨텍스트 자체를 진화시켜 자기개선하는 발상","context 업데이트 정책","W11_opt-Agentic-Context-Engineering_2510.04618.pdf",True),
  ("🔴","ReasoningBank (선택읽기·자기진화)","2025 · arXiv:2509.25140",
   "추론 메모리를 쌓아 에이전트가 진화하는 구조","메모리 항목 추출·재사용","W11_opt-ReasoningBank_2509.25140.pdf",True)]),
12:("프런티어 (개념 위주)","🆕 GUI · 컴퓨터/웹 사용 에이전트",
 ...
 [("🟡","WebArena: A Realistic Web Environment for Agents","Zhou et al., ICLR 2024 · arXiv:2307.13854",
  ...
  ("🟡","Mind2Web: Towards a Generalist Agent for the Web","Deng et al., NeurIPS 2023 · arXiv:2306.06070",
   "실세계 웹사이트 일반화 과제와 데이터","DOM 후보 선택, cross-website 일반화","W12_Mind2Web_2306.06070.pdf",False)]),
13:("프런티어 (개념 위주)","🆕 안전·보안 (Safety & Security)",
 ...
 [("🟡","Not What You've Signed Up For: Indirect Prompt Injection","Greshake et al., AISec 2023 · arXiv:2302.12173",
   "간접 프롬프트 인젝션이 왜 근본적 위협인가","실제 공격 시나리오 분류","W13_Indirect-Prompt-Injection_2302.12173.pdf",False),
  ("🟡","InjecAgent: Benchmarking Indirect Injection in Tool Agents","Zhan et al., ACL 2024 · arXiv:2403.02691",
   "도구 사용 에이전트의 인젝션 취약성 측정","공격 성공률, 방어 프롬프트 효과","W13_InjecAgent_2403.02691.pdf",False)]),
14:("평가와 마무리","평가와 벤치마크 (Evaluation) — 비판적 시각",
 ...
 [("🟢","AI Agents That Matter","Kapoor et al., 2024 · arXiv:2407.01502",
   "리더보드의 함정, 비용을 무시한 정확도의 문제","Pareto(정확도-비용), 재현성","W14_AI-Agents-That-Matter_2407.01502.pdf",False),
  ("🟡","τ-bench: Tool-Agent-User Interaction Benchmark","Yao et al., ICLR 2025 · arXiv:2406.12045",
   "실세계 도메인 도구-에이전트-사용자 상호작용 평가","pass^k, 일관성 측정","W14_tau-bench_2406.12045.pdf",False)]),
15:("평가와 마무리","단순함의 힘 — 에이전트가 꼭 필요한가",
...
16:("평가와 마무리","최종 발표 — 자기 하네스 관점에서 방어",
...
OVERVIEW = {
1:"에이전트의 정의를 잡는 첫 주. 이론에서 **에이전트 vs 워크플로우**의 경계와 자율성 스펙트럼을 다루고, ReAct의 **추론–행동–관찰 루프**를 개념으로 이해한다. 실습에서 그 루프를 while 문으로 직접 구현하고, ReAct·CoT 두 논문으로 '왜 추론과 행동을 엮으면 좋아지나'를 토론한다.",
2:"에이전트가 외부 세계와 만나는 통로인 **도구**를 다룬다. 이론에서 함수 호출 메커니즘·도구 스키마·에러 설계와 **도구 절벽(tool cliff)** 문제를 짚고, 실습에서 도구 등록·파싱·실행을 직접 구현한다. Toolformer·ToolLLM로 모델이 도구 사용을 스스로 학습하는 방식을 본다.",
3:"에이전트가 자기 출력을 스스로 고치는 **반성 루프**. 이론에서 생성→비평→개선 루프와 **언어 피드백이 gradient 없이 작동하는 원리**를 다루고, 실습에서 실패→피드백→재시도를 ReAct에 붙인다. Reflexion·Self-Refine로 자기반성이 진짜 개선인지 검증한다.",
4:"복잡한 과제를 나누고 탐색하는 **계획**. 이론에서 과제 분해, 계획–실행 분리, **트리 탐색(BFS/DFS/MCTS) 직관**을 다루고, 실습에서 트리 분기 추론을 미니 구현한다. Tree of Thoughts·ReWOO로 탐색 비용 대비 성능 이득을 논의한다.",
5:"2025–26 에이전트의 토대인 **추론 모델**. 이론에서 CoT→self-consistency→**test-time compute**의 흐름과, RL로 추론이 창발하는 과정(DeepSeek-R1), STaR 부트스트랩을 다룬다. 실습은 self-consistency 비교. 어렵지 않으면서 화제성이 높아 동기부여가 잘 되는 주.",
6:"외부 지식을 끌어오는 **RAG의 기초**. 이론에서 임베딩·벡터검색, retriever–reader 구조, **컨텍스트 엔지니어링**을 다루고, 실습에서 임베딩→유사도→주입 검색을 직접 만든다 *(최종 프로젝트 부품 1)*. RAG(Lewis)·HyDE.",
7:"검색을 전처리가 아닌 **'도구'로 바꾸는 에이전틱 RAG**. 이론에서 능동 검색(adaptive/self)과 MCP를 다루고, 실습에서 검색을 도구로 노출해 에이전트가 단계별로 호출하게 개조한다 *(최종 프로젝트 부품 2)*. Self-RAG·Adaptive-RAG.",
8:"대화·과제를 넘어 정보를 유지하는 **메모리**. 이론에서 단기/장기, **계층적 메모리(OS 비유)**, 검색 기준(최신성·중요도·관련성)을 다루고, 실습에서 메모리 스트림을 구현한다 *(최종 프로젝트 부품 3)*. MemGPT·MemoryBank.",
9:"환경과 상호작용하며 스킬을 쌓는 **평생학습**. 이론에서 스킬 라이브러리 누적, automatic curriculum, 시뮬 환경 상호작용을 다루고, 실습에서 재사용 가능한 스킬 라이브러리를 만든다. Voyager·Generative Agents.",
10:"⭐ **전환점.** 여러 에이전트의 협업과 프레임워크로의 전환. 이론에서 협업 아키텍처(핸드오프·라우터)와 **그래프 실행 모델**을 다루고, 실습에서 직접 짠 멀티에이전트를 **LangGraph로 재구현·비교**한다(from-scratch→프레임워크 분기점). AutoGen·MetaGPT.",
11:"에이전트를 **RL로 학습**시키는 관점(1주·수식 생략). 이론에서 보상 설계 직관, RLHF→RLVR, 도구학습 보상을 **직관 수준**으로만 다루고, 실습은 보상 기반 도구선택 시뮬레이션. ReTool. *(자기진화 논문은 선택읽기)*",
12:"화면을 보고 클릭하는 **GUI·웹 에이전트**. 이론에서 관찰–행동 루프, **DOM vs 스크린샷**, 벤치마크(WebArena/OSWorld)를 다루고, 실습에서 간단한 웹 태스크 루프를 돌린다. 시각적이라 흥미로운 주. WebArena·Mind2Web.",
13:"행동하는 에이전트의 **안전·보안**. 이론에서 **프롬프트 인젝션(직접/간접)**, 신뢰 경계·최소권한, 가드레일을 다루고, 실습에서 내 에이전트에 가드레일을 붙이고 인젝션을 테스트한다. 실전적이라 난이도 부담이 낮다. Indirect Prompt Injection·InjecAgent.",
14:"에이전트를 **어떻게 평가하나**(비판적 시각). 이론에서 능력 평가 vs 회귀 평가, **비용–정확도 Pareto**, 재현성을 다루고, 실습에서 최종 프로젝트 평가 하네스 골격을 만든다. AI Agents That Matter·τ-bench.",
