...
2:"2025–26 에이전트의 토대인 **추론 모델**을 앞쪽에 배치(Berkeley 흐름). 이론에서 CoT→self-consistency→**test-time compute**와 RL로 추론이 창발하는 과정(DeepSeek-R1), STaR 부트스트랩을 다룬다. **RL은 사전 지식 없음을 가정** — R1 전에 보상 개념을 비유 수준으로 3분 입문(수식 0). 실습은 self-consistency 비교. 유명·readable해 동기부여가 좋다.",
...
6:"외부 지식을 끌어오는 **RAG 기초**. 이론에서 임베딩·벡터검색, retriever–reader 구조를 다루고, 실습에서 임베딩→유사도→주입 검색을 직접 만든다 *(최종 프로젝트 부품 1)*. RAG(Lewis)·HyDE. *(보안 복선: 오늘 만든 '문서 입구'가 W14 간접 인젝션의 공격 통로가 된다 — 예고만.)*",
...
8:"⭐ 컨텍스트를 유한 자원으로 다루는 **컨텍스트 엔지니어링 = 하네스**. 이론에서 LLM이 긴 컨텍스트를 어떻게(못) 쓰는지와 압축·큐레이션을 다루고, 실습에서 컨텍스트 예산·압축을 하네스에 넣는다. Lost-in-the-Middle·LLMLingua. **🏁 중간 데모 체크포인트:** 여기까지가 from-scratch 단일 에이전트의 완성형(루프+도구+계획+반성+RAG+컨텍스트) — 실습 말미에 각자 **'내 에이전트 중간 데모'**(문서 QA를 예산 내 수행)로 중간 점검을 대신한다.",
...
11:"화면을 보고 클릭하는 **컴퓨터·웹 에이전트**. 이론에서 관찰–행동 루프, **DOM vs 스크린샷**, 벤치마크(WebArena/OSWorld)를 다루고, 실습에서 간단한 웹/OS 태스크 루프를 돌린다. 시각적이라 흥미로운 주. WebArena·OSWorld(2024). *(보안 복선: 오늘 붙인 웹 도구가 W14 간접 인젝션의 주 통로다 — 예고만.)*",
...
5:"**추가 모듈:** `reflect.py` — 검증 실패 시 언어 피드백을 붙여 재시도(최대 k회).\n> ✅ **완료:** 처음 틀린 답을 재시도로 고친다.",
6:"**추가 모듈:** `retriever.py` + 샘플 문서 — 청킹→임베딩→코사인 top-k 검색. 📦 *최종 부품*\n> ✅ **완료:** 문서 속 사실을 물으면 관련 청크로 답한다.",
7:"**개조:** `retriever`를 `tools.py`에 **검색 도구**로 등록 → 필요할 때만 검색. 📦 *최종 부품*\n> ✅ **완료:** 상식 질문은 검색 안 하고 문서 질문만 검색한다.",
8:"**추가 모듈:** `context.py` — 토큰 예산 관리(자르기·중요도 정렬·간단 압축).\n> ✅ **완료:** 문서가 많아도 예산 내에서 답 품질을 유지한다.",
9:"**추가 모듈:** `memory.py` — 단기(대화 이력)+장기(파일 저장·검색). 📦 *최종 부품*\n> ✅ **완료:** 이전 세션에 말한 정보를 다음 세션에서 기억한다.",
10:"**개조:** `graph.py` — 지금까지의 루프를 **LangGraph로 재구현**(+선택 비평 노드).\n> ✅ **완료:** from-scratch 버전과 동일 동작을 그래프로 재현·비교한다.",
11:"**추가 모듈:** `tools_web.py` — URL fetch/간단 웹검색 도구(관찰-행동).\n> ✅ **완료:** 웹에서 정보를 가져와 답한다(간단 버전).",
12:"**노트북:** `notebooks/sim_rl.ipynb` — 보상 기반 도구선택 시뮬(앱 통합 X, 개념).\n> ✅ **완료:** 보상 피드백으로 도구 선택이 개선되는 것을 관찰한다.",
