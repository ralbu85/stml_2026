...
2:("부품에서 루프까지","프롬프팅과 추론 (Prompting & Reasoning)",
 ...
 "CoT 프롬프트 작성 + self-consistency(N샘플 다수결) 비교 실험",
...
3:("부품에서 루프까지","도구 사용 (Tool Use)",
 "Anthropic — *Writing Effective Tools for Agents* · MS04",
 "도구 레지스트리·파싱·실행 직접 구현 (계산기 등)",
 ...
 [("🟡","Toolformer: LMs Can Teach Themselves to Use Tools","Schick et al., NeurIPS 2023 · arXiv:2302.04761",
  ...
  ("🔴","ReTool: RL for Strategic Tool Use (선택읽기·RL 보강)","2025 · arXiv:2504.11536",
   "도구 사용 시점·방법을 RL로 최적화하는 핵심 직관","코드 인터프리터 통합, outcome 보상","W03_opt-ReTool_2504.11536.pdf",True)]),
4:("부품에서 루프까지","에이전트 루프 (ReAct)",
 "Lilian Weng — *LLM Powered Autonomous Agents* · Berkeley 에이전트 역사 강의",
 "ReAct(Thought→Action→Observation) while 루프 직접 구현 — W3 도구 레지스트리 위에서 + 미니 evalset",
 ...
 [("🟢","ReAct: Synergizing Reasoning and Acting in LLMs","Yao et al., ICLR 2023 · arXiv:2210.03629",
   "Think–Act–Observe 루프 구조와 추론·행동을 엮는 이유","HotpotQA·ALFWorld 셋업","W04_ReAct_2210.03629.pdf",False),
  ("🟡","DeepSeek-R1: Incentivizing Reasoning via RL","DeepSeek-AI, 2025 · arXiv:2501.12948",
   "순수 RL로 추론이 창발하는 큰 그림('aha moment')과 추론 모델이 루프 설계에 미치는 영향","GRPO, cold-start 데이터","W04_DeepSeek-R1_2501.12948.pdf",False),
  ("🟡","STaR: Self-Taught Reasoner (선택읽기·추론 학습 계보)","Zelikman et al., NeurIPS 2022 · arXiv:2203.14465",
   "스스로 만든 추론으로 추론을 부트스트랩하는 아이디어","rationalization 트릭","W04_opt-STaR_2203.14465.pdf",True)]),
5:("지식","검색 증강 (RAG) — 1부: 기초",
 ...
 [("🟡","Retrieval-Augmented Generation for Knowledge-Intensive NLP","Lewis et al., NeurIPS 2020 · arXiv:2005.11401",
   "파라메트릭 vs 비파라메트릭(검색) 지식 결합","RAG-Sequence vs Token, retriever 공동학습","W05_RAG-Lewis_2005.11401.pdf",False),
  ("🟡","HyDE: Precise Zero-Shot Dense Retrieval without Labels","Gao et al., ACL 2023 · arXiv:2212.10496",
   "가설 문서를 생성해 검색 품질을 올리는 발상","dense retriever와의 결합","W05_HyDE_2212.10496.pdf",False)]),
6:("지식","에이전틱 RAG + 프로토콜 (MCP)",
 ...
 [("🟡","Adaptive-RAG: Adapting Retrieval to Query Complexity","Jeong et al., NAACL 2024 · arXiv:2403.14403",
   "질의 난이도에 따라 검색 전략을 바꾸는 에이전틱 구조","complexity classifier","W06_Adaptive-RAG_2403.14403.pdf",False),
...
2:"에이전트의 추론 능력을 프롬프트로 끌어내는 주. 이론에서 messages·역할 구조와 few-shot(in-context learning), **CoT**(생각의 외부화), **test-time compute**(self-consistency 다수결), 추론 모델(o1/R1)의 사용 관점(thinking budget·비용)을 다룬다. 실습은 CoT·self-consistency 비교 실험. 추론 모델의 훈련 원리(STaR·RLVR)는 W4 발표·선택읽기로 위임한다. **첫 학생 발표 주 — 부담 완화를 위해 1편만.**",
3:"에이전트가 외부 세계와 만나는 통로인 **도구**. 이론에서 function calling 파이프라인(스키마→호출 의도→파싱→실행→재주입)·스키마 설계·에러 설계와 **도구 절벽**을 다루고, 실습에서 도구 레지스트리·파싱·실행을 직접 만든다. 다음 주 이 레지스트리 위에서 루프가 돈다. Toolformer·ToolLLM.",
4:"추론(W2)과 도구(W3)를 하나의 제어 루프로 결합하는 주. 이론에서 그냥 묻기→CoT→Act-only→**ReAct의 추론–행동–관찰 루프** 계보를 다루고 트레이스를 라이브로 읽는다. 실습에서 그 루프를 while 문으로 직접 구현해 **첫 완전한 에이전트**를 완성하고, 이후 매주 쓰는 **미니 evalset**(5문항)을 시작한다. ReAct·DeepSeek-R1. R1 발표는 RL 사전 지식을 가정하지 않는다 — 발표 필수는 보상 서사의 큰 그림까지.",
5:"외부 지식을 끌어오는 **RAG 기초**. W4의 에이전트는 돌지만 아는 것이 없다 — 이론에서 임베딩·벡터검색, retriever–reader 구조를 다루고, 실습에서 임베딩→유사도→주입 검색을 직접 만든다 *(최종 프로젝트 부품 1)*. RAG(Lewis)·HyDE. *(보안 복선: 오늘 만든 '문서 입구'가 W14 간접 인젝션의 공격 통로가 된다 — 예고만.)*",
6:"검색을 전처리가 아닌 **'도구'로 바꾸는 에이전틱 RAG**와 **MCP**. 이론에서 능동 검색(adaptive/self)과 프로토콜을 다루고, 실습에서 검색을 W3 도구 레지스트리에 등록해 에이전트가 단계별로 호출하게 개조한다 *(최종 프로젝트 부품 2)*. Adaptive-RAG·Self-RAG — Self-RAG는 학기 유일의 중급~심화 논문으로, 발표자 사정에 따라 후주(W9 이후) 이동이 가능하다(디커플링).",
...
2:"**추가 모듈:** `reasoning.py` — CoT 프롬프트 토글 + 같은 질문을 N번 샘플→다수결(self-consistency).\n> ✅ **완료:** 애매한 질문에서 단일 답보다 정확도가 오른다.",
3:"**추가 모듈:** `tools.py` — 도구 등록(dict)·JSON 액션 파싱·실행(계산기·문자열검색).\n> ✅ **완료:** 모델의 도구 호출 출력을 파싱·실행해 결과를 재주입하는 단발 왕복이 성공한다. (반복 루프는 W4)",
4:"**추가 모듈:** `loop.py` — ReAct while 루프 골격. W3의 `tools.py` 레지스트리를 호출한다. + 미니 evalset 5문항.\n> ✅ **완료:** 루프가 계산기 도구를 실제로 호출해 답하고, 미니 evalset 결과가 기록된다.",
5:"**추가 모듈:** `retriever.py` + 샘플 문서 — 청킹→임베딩→코사인 top-k 검색. 📦 *최종 부품*\n> ✅ **완료:** 문서 속 사실을 물으면 관련 청크로 답한다.",
6:"**개조:** `retriever`를 `tools.py`(W3)에 **검색 도구**로 등록 → W4 루프가 필요할 때만 검색. 📦 *최종 부품*\n> ✅ **완료:** 상식 질문은 검색 안 하고 문서 질문만 검색한다.",
