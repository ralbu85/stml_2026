...
Shipped by OpenAI (2025) and Google Gemini (2024); Perplexity runs a lighter variant. Devin does the same for a software ticket, and computer-use agents (OpenAI Operator, Anthropic computer use) do it with the screen and mouse as their action surface.
...
- The autonomy trade arrives here at full strength: slow, expensive, hardest to predict — which is why these ship with confirmation prompts for consequential actions.
...
Two questions the spectrum cannot answer, because neither is about control flow:

- ChatGPT holds a conversation; Deep Research runs unattended for minutes. Is only one of them an agent?
- Copilot's suggestion appears inside the editor; Devin's arrives as a pull request. Is one of them more of an agent?

Both ask how the system meets its user, not who decides its next action. That is a second axis, and it is independent of the first.
...
"Build a workflow instead" is unusable advice while the only workflow on offer is Band 1's straight line. The fixed-control-flow side has a design vocabulary of its own.

**Workflow pattern** = a named arrangement of LLM calls whose order is decided by the code, not by the model.
...
Three of the five are not pipelines: a fixed procedure is not the same thing as a simple one. Each returns in the chapter that builds it.
