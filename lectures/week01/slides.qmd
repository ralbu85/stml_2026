    ...
    include-in-header:
      text: |
        <style>
          ...
          .reveal .divider { padding-top: 6%; }
          .reveal .divider h2 { font-size: 1.75em; color: #1a3a5c;
            border-bottom: 3px solid #1a3a5c; padding-bottom: .22em; margin-bottom: .55em; }
          .reveal .divider .eyebrow { color: #6b7f96; font-weight: 600;
            letter-spacing: .14em; font-size: .78em; text-transform: uppercase; }
          .reveal .divider .left { color: #8a6a4a; }
          .reveal .divider p { font-size: 1.02em; }
        </style>
---
...
[Where this starts.]{.left} Two products built on the same model do entirely different work. The difference is not in the weights, so it has to be in the program around them.

This part establishes what that program does, and why a single call is not enough to do it — the shift from asking for a finished answer to executing a task in steps.

**It answers:** what changes when work is done across passes rather than in one.
...
[What Part 1 left open.]{.left} Steps help — but Part 1 never said *who decides what the next step is*. That decision is the entire design question, and the industry does not even agree on what to call a system that makes it.

This part supplies the axis (control flow), a scale to place a system on it, the boundary below which nothing is an agent, and a way to read the right level off the task instead of off preference.

**It answers:** how much authority to hand over, and how to tell.
...
[What Parts 1–2 left open.]{.left} They answer *how much* to hand over. They say nothing about *what to build*.

A system anywhere on the scale — a router, a multi-step agent, a fleet — is assembled from the same four parts. This part names them and states what each is responsible for, and what fails when one is missing.

**It answers:** what an agent is made of.
...
[Why this comes before the agent patterns.]{.left} Part 2 ended with a rule — hand over only as much as the task requires — and Anthropic states it more bluntly still: find the simplest solution possible, which may mean not building an agentic system at all.

That rule is unusable while the only alternative on offer is a straight line of calls. This part is the vocabulary of the low-autonomy side: five arrangements in which **the code, not the model, decides the order**.

**It answers:** everything worth building *before* handing over control flow.
...
[How this differs from Part 4.]{.left} Part 4 arranged calls, with the code holding the order. This part is the other axis: what a system is made able to **do** once the model does hold some of the flow — judge its own output, reach outside the weights, plan a route, divide the work.

Composition and capability are independent, and production systems combine them freely.

**It answers:** what capability to engineer, once autonomy has been granted. These four are also this course's syllabus — Ch. 3, 5, 6, 7.
...
[What is still missing.]{.left} Parts 3 to 5 are design vocabulary, and vocabulary proves nothing.

Two questions remain. Where has any of this actually paid off with users — and what keeps an autonomous loop from running away, or from reporting a success it never achieved?

**It answers:** the evidence, and the boundary that makes the loop safe to run.
