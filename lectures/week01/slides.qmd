...
Every band is occupied by shipping products. The next four slides take one band each: what the service does, and which decisions it holds.
...
Shipped by OpenAI (2025) and Google Gemini (2024); Perplexity runs a lighter variant.

- Across those minutes the model decides everything the spectrum's right end implies: *which searches to run, which pages to open, which to trust, when the evidence suffices.*
...
**Support agent** = a system placed on a company's help desk that reads an incoming customer message and settles it — answering the question, performing an account action such as a refund, or handing the conversation to a human.

Sold to companies as the first tier of customer service: Intercom Fin, Sierra (2024–).
