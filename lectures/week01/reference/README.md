...
| `openai-practical-guide-to-building-agents.txt` (+ `.pdf`) | OpenAI, *A Practical Guide to Building Agents* (2025), 34 pp. | The three core components — Model, Tools, Instructions — quoted (§1.3). The three tool types: data, action, orchestration (§1.3). The instruction best practices: use existing documents, specify edge cases, keep modular (§1.3). The exclusions — chatbots, single-turn calls, sentiment classifiers (§1.2). The tool-overload and complex-logic triggers for splitting into multiple agents, with the 15-distinct / 10-overlapping figures (§1.5). |
| `ng-agentic-ai-M1-intro-slides.txt` (+ `.pdf`) | Andrew Ng, *Agentic AI* (DeepLearning.AI), Module 1 — learner slides, 39 pp., from `materials/repos/agentic_ai_andrew/` | The zero-shot vs. agentic contrast and the "without using backspace" image, and the definition of an *agentic AI workflow* (§1.1, Figure 1.1). **The position this chapter takes on the definition dispute** — degrees of agenticness (§1.2). The three autonomy bands (§1.2, Table 1.2). The easier-vs-harder task dimensions (§1.2). All four design patterns with their illustrations: the Coder/Critic reflection exchange, the tool categories, HuggingGPT planning, and the Multiagent Debate figures (§1.5). |
| `smolagents-intro-agents.md` | Hugging Face, smolagents conceptual guide — [link](https://huggingface.co/docs/smolagents/conceptual_guides/intro_agents) | The levels-of-agency table with its code column, reproduced (§1.2, Table 1.1, Figure 1.3). The explicit rejection of the binary definition, quoted (§1.2). The ReAct Thought–Action–Observation framing (§1.5). |
...
Figures. Figure 1.2 (autoregressive generation) is P.-M. Dartus, *How LLMs Generate
Text for the Rest of Us* (2025), pm.dartus.fr — the image itself, credited in place.
Figures 1.1, 1.3, 1.4 and 1.5 were drawn for this course from the structures in the
sources above (Ng's essay contrast, the smolagents levels, Anthropic's five patterns,
and Anthropic's loop properties); each carries its attribution inside the drawing.
