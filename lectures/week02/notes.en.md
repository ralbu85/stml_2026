...
**Homework.** `W2_hw_prompting.ipynb` — collected via the LMS and auto-graded from the saved outputs: four assignments in the lab's structure with new tasks (chain-of-thought by instruction on two word problems, an exemplar that pins a `DATE:` output format, your own chain-of-thought prompt with a five-sample vote, few-shot chain-of-thought exemplars on a Roman-numeral evalset, ≥ 5/6). Due before W3.
