...
**Lab.** `W2_lab_prompting.ipynb` — adapted nearly as given from Anthropic's *Prompt Engineering Interactive Tutorial* (ch. 3–8) and *Prompt Evaluations* course (lesson 3), every call written out as in W1 with no helper functions: roles and delimiters, thinking-step-by-step flips on the source's own examples (including the silent-thinking control), then a code-graded eval improved from baseline through a format fix to your own chain-of-thought prompt (target ≥ 11/12), the few-shot email-classification exercise (4/4), self-consistency (Wang et al., 2022) on the hardest eval item, and the hallucination limit of §2.4. The lab is collected: four graded assignments close it (CoT by instruction, format pinned by an exemplar, a self-written vote, few-shot CoT exemplars on a Roman-numeral evalset). Reference answers: `labs/checkpoints/week02/solution.py`.
