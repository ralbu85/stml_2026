...
**Lab.** `W2_lab_prompting.ipynb` — every call written out as in W1 with no helper functions. It opens by reproducing the measurement of §2.1 with code: the apple problem, then six warehouse problems with three-digit products where answer-only fails and the worked solution recovers, followed by the silent-thinking and answer-first controls of §2.2 (Wei et al., 2022, Figure 1 and ablation). The rest adapts Anthropic's *Prompt Engineering Interactive Tutorial* (ch. 6–8) and *Prompt Evaluations* course (lesson 3) nearly as given: a code-graded eval improved from baseline through a format fix to your own chain-of-thought prompt (target ≥ 11/12), the few-shot email-classification exercise (4/4), self-consistency (Wang et al., 2022) on the hardest eval item, and the hallucination limit of §2.4. The lab is collected: four graded assignments close it (CoT by instruction, format pinned by an exemplar, a self-written vote, few-shot CoT exemplars on a Roman-numeral evalset). Reference answers: `labs/checkpoints/week02/solution.py`.
