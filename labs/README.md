...
| 2 | [`W2_lab_prompting.ipynb`](../lectures/week02/W2_lab_prompting.ipynb) — wrapper-free like W1 (raw `create` calls, one condition per cell, *Try:* prompt variations): answer-only vs worked solution on multi-step arithmetic graded by code, silent-thinking + answer-first controls, a code-graded eval (CoT ≥ 11/12), few-shot (email 4/4), self-consistency, hallucination limit; in-class only, not collected · **hw [`W2_hw_prompting.ipynb`](../lectures/week02/W2_hw_prompting.ipynb) — collected via the LMS, graded by `tests/grade_hw.py`: 4 assignments (CoT instruction ×2 problems, exemplar-pinned `DATE:` ×2, own CoT prompt + 5-sample vote, few-shot CoT on Roman numerals ≥ 5/6)** · [`W2_hw_build_evalset.ipynb`](../lectures/week02/W2_hw_build_evalset.ipynb) — own 8-item evalset, CoT beats answer-only ≥ 7/8 (status pending) | Wei et al. (2022) Fig. 1 + ablation reproduced on course problems; Anthropic *Prompt Eng. Interactive Tutorial* ch. 6–8 + *Prompt Evaluations* lesson 3 (near-verbatim) | ✅ |
