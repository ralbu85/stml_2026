...
> **목표:** 매주 from-scratch로 **모듈 하나**씩 추가해, 16주 뒤 **연구보조 검색·종합 에이전트(`docqa-agent`)** 를 완성한다.
...
**"감싸기(wrap), 버리기 아님."** from-scratch로 짠 코드를 W11에서 LangGraph 노드의 '속'으로 재사용한다.

| 구간 | 빌드 베이스 |
|---|---|
| **W1–10** | **from-scratch** — `retriever.py`·`memory.py`·`tools.py` 등을 직접 구현 (LLM 호출만 aisuite) |
| **W11** | 🔄 **전환점** — 위 모듈들을 **LangGraph 노드로 감싼다**. 오케스트레이션·상태만 프레임워크에 위임 |
| **W12–15** | **LangGraph 위에서 확장** — 웹도구·가드레일=노드, eval=바깥에서. **W1–10 모듈 그대로 재사용** |

> **왜 후반이 더 쉬워지나:** 멀티에이전트·메모리 연결·웹도구 같은 복잡한 후반 기능은 그래프/상태 모델로 하면 from-scratch보다 간단하다. 힘든 로직은 이미 W1–10에서 손으로 짜서 이해한 상태이므로, W11의 부담은 *LangGraph 기본 문법 1회 학습*뿐이다.
> **대안:** 프레임워크 의존을 피하려면 W11을 비교용 1회 실습으로만 두고 W12–16을 from-scratch 본선으로 유지할 수 있다(후반 난이도 ↑).
...
| 12 | `tools_web.py` | URL fetch/웹검색 도구(arXiv 등) | 웹에서 정보 가져와 답(간단) |
...
> 스캐폴드는 [`labs/`](../labs/)에서 재구축 예정 — 2026-07-13 전면 초기화(직전 상태는 git `079ab0a`). 재구축 표준: Andrew Ng *Agentic AI* 실습 스타일(주차 노트북 = 학습목표→Setup→관찰(빈칸 없음)→구현(명세+TODO)→조립→측정→연습문제; 주당 프롬프트 작성형 과제 1개 이상; 오프라인 pytest + 실LLM 스모크 채점 병행).
